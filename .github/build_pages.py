...
ROOT=Path(__file__).resolve().parents[1]; ARCHIVE=ROOT/"Mangareader-final.zip"; BUILD=ROOT/".build"/"Mangareader"; DIST=ROOT/"dist"
...
with ZipFile(ARCHIVE) as z:
    for info in z.infolist():
        ...
        if not name.startswith("Mangareader/") or name.startswith("Mangareader/chapter-images/") or name.startswith("Mangareader/manga-covers/"): continue
...
for name in ["data/content.json","data/manual-chapters.json","data/extra-chapters.json","data/chapter-dates.json","data/upcoming-chapters.json","data/ads-config.json"]:
    source=ROOT/name
    if source.exists():
...
(DIST/".nojekyll").touch()
print(f"Built {DIST} successfully; scan/covers excluded and repository data preserved.")
