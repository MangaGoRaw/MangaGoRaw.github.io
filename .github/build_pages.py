...
for path in BUILD.rglob("*"):
    ...
    if path.name == "analytics.js":
        text="""(function(){'use strict';var ID='G-HC32QHLNXB';var isChapter=location.pathname==='/chapter.html'||location.pathname==='/chapter';var debug=new URLSearchParams(location.search).get('ga_debug')==='1';if(!window.__mangaGoRawGA){window.__mangaGoRawGA=true;window.dataLayer=window.dataLayer||[];window.gtag=function(){dataLayer.push(arguments)};gtag('js',new Date());gtag('config',ID,{send_page_view:false});var g=document.createElement('script');g.async=true;g.src='https://www.googletagmanager.com/gtag/js?id='+encodeURIComponent(ID);document.head.appendChild(g)}function sendPageView(p){p=p||{};var payload={page_title:p.page_title||document.title,page_location:p.page_location||location.href,transport_type:'beacon'};if(debug)payload.debug_mode=true;if(window.gtag)window.gtag('config',ID,payload)}function load(src,id){if(document.querySelector('script[data-mangagoraw-'+id+']'))return;var s=document.createElement('script');s.src=src;s.async=false;s.setAttribute('data-mangagoraw-'+id,'true');document.head.appendChild(s)}function boot(){if(isChapter){load('/chapter-navigation.js?v=2','navigation');return}sendPageView({page_title:document.title,page_location:location.href})}if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot,{once:true});else boot();window.mangaGoRawAnalytics={pageView:sendPageView,event:function(n,p){if(window.gtag)window.gtag('event',n,p||{})}}})();\n"""
